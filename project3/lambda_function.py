...
def lambda_handler(event, context):
  try:
    ...
    #
    #Initialize a dictionary to count occurrences of first non-zero digits
    first_non_zero_digit_count = {str(i): 0 for i in range(1, 10)}
    ...
    for i in range(0, number_of_pages):
      ...
      for word in words:
        ...
        if word.isnumeric():
          ...
          for char in word:
            if char != '0':
              first_non_zero_digit_count[char]+=1
              break
    ...
    #
    outfile.write("0 0\n")
    for digit, count in first_non_zero_digit_count.items():
    ...
    update_query="UPDATE jobs SET status = 'completed', resultsfilekey = %s WHERE datafilekey = %s"
    
    # Execute the update query for success case
    num_rows_modified=datatier.perform_action(dbConn, update_query, [bucketkey_results_file, bucketkey])
    if num_rows_modified == 0:
      raise Exception("Update failed, no rows were modified.")
  ...
  #
    ...
    #
    dbConn = datatier.get_dbConn(rds_endpoint, rds_portnum, rds_username, rds_pwd, rds_dbname)
    update_query="UPDATE jobs SET status = 'error', resultsfilekey = %s WHERE datafilekey = %s"
    
    # Execute the update query for error case
    num_rows_modified=datatier.perform_action(dbConn, update_query, [bucketkey_results_file, bucketkey])
    if num_rows_modified == 0:
      raise Exception("Update failed, no rows were modified.")
